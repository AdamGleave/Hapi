%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{tikzpicture}[nodes={font=\bf\small}, scale=0.45]
%  \draw [help lines] (-1,-1) grid (8,5);o

% epsilon=10
% Q=[C,B]
% discharge C
% relabel C
% Pi(C) = 0 + 1 + 10 = 11
% Q=[B,C]

\tikzstyle{mynode}=[%
  draw,
  circle,
  minimum width=0.4,
  inner sep=0.5mm,
]

\tikzstyle{myarc}=[%
  draw,
  arrows={-Triangle[scale=1]},
  line width=1pt,
]

% nodes
\node [mynode] (a) at (0, 4) {A};
\node [mynode] (b) at (4, 8) {B};
\node [mynode] (c) at (4, 0) {C};
\node [mynode] (d) at (8, 4) {D};

% excesses
\node [left=0.55cm] at (0,4.5) {$e_A = 0$}; % A
\node [above=0.6cm] at (4,8) {$e_B = 2$}; % B
\node [below=0.2cm] at (4,0) {$e_C = 1$}; % C
\node [right=0.3cm] at (8,4.5) {$e_D = -3$}; % D

% potentials
\node [left=0.3cm] at (0,3.5) {$\pi_A = 11$}; % A
\node [above=0.15cm] at (4,8) {$\pi_B = 11$}; % B
\node [below=0.65cm] at (4,0) {\color{red}$\pi_C = 11$}; % C
\node [right=0.3cm] at (8,3.5) {$\pi_D = 0$}; % D

\path [myarc] (b) -- node[above, sloped] {$(2,-1)$} (a);
\path [myarc] (a) -- node[below, sloped] {\color{red}$(2,5)$} (c);
\path [myarc] (c) edge[bend right=20, looseness=1] node[above, sloped] {\color{red}$(1,-5)$} (a);
\path [myarc] (b) -- node[right] {\color{red}$(5,1)$} (c);
\path [myarc] (b) -- node[above, sloped] {$(1,-1)$} (d);
\path [myarc] (c) -- node[below, sloped] {\color{red}$(5,-10)$} (d);

\end{tikzpicture} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
