...
% excesses
\node [left=0.55cm] at (0,4.5) {\color{red} $e_A = 0$}; % A
\node [above=0.6cm] at (4,8) {\color{red} $e_B = 2$}; % B
\node [below=0.2cm] at (4,0) {\color{red} $e_C = 1$}; % C
...
\path [myarc,red] (b) -- node[above, sloped] {$(2,10)$} (a);
\path [myarc,red] (a) -- node[below, sloped] {$(2,-6)$} (c);
\path [myarc,red] (c) edge[bend right=20, looseness=1] node[above, sloped] {$(1,6)$} (a);
