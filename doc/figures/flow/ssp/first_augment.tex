...
% excesses
\node [left=0.3cm] at (0,4.5) {\color{red} $e_A = 2$}; % A
\node [above=0.6cm] at (4,8) {$e_B = 0$}; % B
\node [below=0.2cm] at (4,0) {$e_C = 0$}; % C
\node [right=0.3cm] at (8,4.5) {\color{red} $e_D = -2$}; % D
...
% (cap,cost)
\path [myarc, red] (a) -- node[above, sloped] {$(1,0)$} (b);
\path [myarc, red] (b) edge[bend left=20, looseness=1] node[below, sloped] {$(1,0)$} (a);
\path [myarc] (a) -- node[below, sloped] {$(3,0)$} (c);
\path [myarc] (b) -- node[right] {$(5, 6)$} (c);
\path [myarc, red] (d) -- node[above, sloped] {$(1,0)$} (b);
