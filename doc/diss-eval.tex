...
%XXX: Flag up that preemption is not supported?
...
\todo{XXX: Bump this up to full size for relaxation; augmenting path you'll need to keep at medium. Flag up difference in full performance in text.}
\begin{figure}
    \begin{widepage}
        ...
        \end{subfigure}
        \begin{subfigure}[c]{\textwidth}
            \includegraphics{inc/same_relax_medium_cdf_2col}                        \includegraphics{inc/same_relax_medium_incremental_only_cdf_2col}
...
Cost scaling is the fastest full algorithm on flow scheduling networks. \Cref{fig:inc-head-to-head-my} pits my incremental algorithm against my implementation of cost scaling. The incremental algorithm considerably outperforms cost scaling, with a speedup of 19x on the full (12,000 machine) Google cluster. Note this is, however, significantly less than the 450x speedup observed in the previous section. This is to be expected, as cost scaling is considerably faster on the full problem.
