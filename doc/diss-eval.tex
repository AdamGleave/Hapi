...
I developed unit tests for each component, which were re-run as a regression test at the end of each development cycle in accordance with the spiral model (see \cref{sec:prep-management-model}). Standard unit test frameworks are not designed to work with large, external datasets such as flow networks. Accordingly, I developed my own test harness in Python to automate verification of flow algorithms. I wrote unit tests for smaller components, such as data structures, using the GTest framework (see \cref{sec:prep-tools-libraries}).
...
%FIGURE: Illustrate timeline of events, if time
...
\todo{XXX: Include or not? This somewhat diminishes my result, but it's a good point}
...
To investigate this, I extended the reference implementation of the relaxation algorithm, RelaxIV~\cite{RelaxIV:2011}, to support operating incrementally (see \cref{sec:impl-incremental-impl}). For the from scratch solver, I selected Goldberg's heavily optimised implementation of his cost scaling algorithm, CS2~\cite{CS2:2009}. This has been found to be one of the fastest flow solvers in independent benchmarks~\cite{KiralyKovacs:2012}, corroborated by my own tests.

The Quincy system also used CS2. Isard, \textit{et al.}\ report a scheduling latency of just over a second on a 2,500 machine simulated cluster~\cite{Isard:2009}. \Cref{fig:inc-head-to-head-optimised:large} shows the results of my experiment on a similarly sized cluster, in which CS2 achieves a scheduling latency of just over \SI{500}{\milli\second}. Given that the experiment by Isard, \textit{et al.}\ dates to 2009, this $2\times$ performance improvement is unsurprising. My incremental solver is faster at every percentile, and achieves a $12\times$ average speedup.
