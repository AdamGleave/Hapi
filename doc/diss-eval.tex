...
%WORDCOUNT: Don't really need subsections here
...
\begin{figure}
    ...
    \end{subfigure}
    \caption[Firmament scheduling tasks]{Firmament scheduling tasks using Hapi}
...
Two versions of small heap were implemented. To support decreasing the key of elements in the priority queue (a common operation in Djikstra's algorithm), it is necessary to maintain a \emph{reverse index} mapping vertex IDs to elements in the priority queue. In the big heap, this was implemented using an \emph{array} of length $n$. For the small heap, it is possible to reduce memory consumption by using a \emph{hash table}, storing mappings only for vertices $v$ present in the priority queue.
...
\Cref{table:general-datasets} summarises the datasets used in this test. A 1000 networks were generated of each type, by varying the random seed. 250 of these networks were used for training, with the remaining 750 being test data.
...
Relaxation is significantly faster than successive shortest path, both as a from scratch and incremental solver\footnotemark. The mean scheduling latency shown in \cref{fig:inc-same:relax-my} is significantly lower than that in \cref{fig:inc-same:ssp}, despite the test dataset of the former being some $5\times$ larger than that of the latter. However, the performance of relaxation is known to be highly dependent on the problem instance (see \cref{sec:impl-relax}), so this result may not hold for cost models invented in the future. Incremental successive shortest path is a viable alternative in this case.
\footnotetext{In fact, relaxation is competitive with algorithms such as cost scaling on this class of flow network.}

Both incremental solvers have a long tail. This is a reflection of the distribution of events in the cluster trace. In most scheduling rounds, there are only a handful of changes, and the incremental solver returns quickly. Occasionally, however, thousands of events may occur in a small time window such as when a large job is submitted. In this case, reoptimisation may take a while, although it is still faster than solving from scratch.
...
Cost scaling is the fastest from scratch algorithm on flow scheduling networks. \Cref{fig:inc-head-to-head-my} pits my incremental algorithm against my implementation of cost scaling. The incremental algorithm considerably outperforms cost scaling, with a speedup of $18\times$ on the full (12,000 machine) Google cluster. Note this is, however, significantly less than the $450\times$ speedup observed in the previous section. This is to be expected, as cost scaling is considerably faster on the from scratch problem.
