...
The successive shortest path and relaxation algorithm described in \cref{sec:impl-ssp} and \cref{sec:impl-relax} are adapted to support operating in an incremental mode. Note that considerable research effort has gone into producing efficient implementations of the relaxation algorithm. The reference implementation, RelaxIV, is due to Bertsekas~\cite{BertsekasCodes:1988}, and has been ported from Fortran to C++ by Frangioni~\cite{RelaxIV:2011}. In addition to modifying my own implementation of the relaxation algorithm, I have also adapted RelaxIV, for use in comparative evaluation (see \S\ref{sec:eval-comparative}).

First, I describe how reduced cost optimality may be maintained as the network is updated, a prerequisite for using these algorithms in an incremental mode. Next, I give the details of how my implementations have been adapted for incremental usage. Finally, I discuss how RelaxIV was modified.

\subsection{Maintaining reduced cost optimality}
% TODO: Finish lifting this up
Augmenting path algorithms require that $\left(\mathbf{x},\boldsymbol{\pi}\right)$ satisfy reduced-cost optimality. Recall this is defined in \cref{eq:optimality-reduced-cost} as:
\[c_{ij}^{\boldsymbol{\pi}}\geq0\:\forall(i,j)\in E_{\mathbf{x}}\]

Normally, this condition is satisfied upon entering the routine by initialising both $\mathbf{x}$ and $\boldsymbol{\pi}$ to $\mathbf{0}$. It is maintained subsequently as an invariant, see \S\ref{sec:impl-ssp-analysis} and \S\ref{sec:impl-relax-analysis}. The initialisation is skipped when using the algorithm to reoptimise, so it is necessary that the reduced cost optimality conditions are satisfied prior to the algorithm being invoked.

Fortunately, there is a simple method to maintain reduced cost optimality as the network is updated. Note that we cannot, in general, hope to also maintain feasibility\footnotemark: otherwise, this routine would itself solve the incremental problem! I describe below how the flow is to be modified to preserve optimality, in response to each possible type of network change. Note that this method modifies only the flow, and not potentials.
...
Note that an arc with zero capacity is equivalent to their being no such arc: it is never present in the residual network (nor is its reverse), and always has a flow of zero. Thus, it is possible to model addition of an arc $(i,j)$ with cost $c_ij$ and capacity $u_ij$ in terms of increasing the capacity of an arc $(i,j)$ with cost $c_ij$ from $0$ to $u_ij$. Similarly, removing an arc $(i,j)$ corresponds to decreasing the capacity to $0$. Maintaining reduced cost optimality in the presence of these capacity changes is described in the following section.

\subsubsection{Changing the capacity of an arc}

Changing the capacity of an arc $(i,j)$ from $u_ij$ to $u'_ij$ does not change $c_ij$ or the potentials $\boldsymbol{\pi}$. The reduced cost $c_{ij}^{\boldsymbol{\pi}}$ therefore remains unchanged.

When decreasing the capacity of an arc, we set $x_ij \gets \min\left(x_ij,u'_ij\right)$ to ensure the capacity constraints in \cref{eq:capacity-constraints} continue to hold\footnotemark. Reduced cost optimality is always preserved. Suppose the arc is not present in the residual network prior to the update: that is, $(i,j) \not \in E_\mathbf{x}$. Then the arc was previously saturated with $x_ij = u_ij$, and we will now have $x_ij = u'_ij$. Thus the arc remains saturated, and so $(i,j) \not \in E_\mathbf{x}$ continues to hold. Otherwise, if $(i,j) \in E_\mathbf{x}$, then as reduced cost optimality was satisfied prior to the update it follows that $c_{ij}^{\boldsymbol{\pi}} \geq 0$. It may be the case that $x_ij \geq u'_ij$, in which case $(i,j) \not \in E_\mathbf{x}$ after the update, which is harmless. Otherwise, $(i,j) \in E_\mathbf{x}$ and $c_{ij}^{\boldsymbol{\pi}} \geq 0$ continues to hold, satisfying the optimality conditions.
...
There are two cases when increasing the capacity of an arc. If $c_{ij}^{\boldsymbol{\pi}} \geq 0$, then no action need be taken. However, if $c_{ij}^{\boldsymbol{\pi}} < 0$ then we must set $x_ij \gets u'_ij$, so that $(i,j)$ remains absent from the residual network.

\subsubsection{Changing the cost of an arc}

Changing the cost of an arc $(i,j)$ from $c_ij$ to $c'_ij = c_ij + \delta$ will also change the reduced cost from $c_{ij}^{\boldsymbol{\pi}}$ to $c'_{ij}^{\boldsymbol{\pi}} = c_{ij}^{\boldsymbol{\pi}} + \delta$. It is simplest for us to consider the complementary slackness conditions, which are equivalent to reduced cost by \cref{thm:optimality-complementary-slackness}.

\paragraph{Case $c'_{ij}^{\boldsymbol{\pi}} > 0$}
In this case, we require $x_ij = 0$. This will already be the case if $c_{ij}^{\boldsymbol{\pi}} > 0$, but otherwise it may require a flow update.

\paragraph{Case $c'_{ij}^{\boldsymbol{\pi}} = 0$}
This case is a no-op. Complementary slackness is satisfied for all values of $x_ij$ satisfying the capacity constraints $0 \leq x_ij \leq u_ij$.

\paragraph{Case $c'_{ij}^{\boldsymbol{\pi}} < 0$}
In this case, we require $x_ij = u_ij$. This will already be the case if $c_{ij}^{\boldsymbol{\pi}} < 0$, but otherwise it may require a flow update.

\subsection{Implementations}
% Updating data structures: e.g. need to maintain a map. Could mention this for Frangioni, certainly. Perhaps say my implementation relatively simple, since was designed from the outset with this flexibility in mind.
% Beyond this? Not much to say

Detail the implementations you've made. AP, RELAX, RELAX Frangioni. Not sure how much there is to say. Could possibly chat about data structures needed for mappings.
