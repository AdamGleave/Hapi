...
%\begin{lemma} \label{lemma:relax-rc-implies-w-optimal}
%Let $\mathbf{x}$ be a pseudoflow satisfying the reduced cost optimality conditions for some potentials $\boldsymbol{\pi}$. Then $\mathbf{x}$ is an optimal solution to $\mathrm{LR}(\boldsymbol{\pi})$.
%\end{lemma}
%\begin{proof}
%Consider the form of $w(\boldsymbol{\pi})$ stated in terms of reduced cost and balances, given in \cref{eq:relax-obj-fun-balance}. The second sum, $\sum_{i \in V} \pi_i b_i$, is constant in $\mathbf{x}$. The first sum is minimised (subject to the capacity constraints given in \cref{eq:relax-capacity-constraints}) when the complementary slackness optimality conditions given in \cref{thm:optimality-complementary-slackness} are satisfied. But reduced cost optimality and complementary slackness optimality are equivalent, and thus since $\left(\mathbf{x},\boldsymbol{\pi}\right)$ is reduced cost optimal we have:
%
%\[\mathbf{x} = \argmin_{x}\left[\sum_{\left(i,j\right)\in E}c_{ij}^{\boldsymbol{\pi}}x_{ij}+\sum_{i\in V}\pi_{i}b_{i}\right]\]
%
%subject to \cref{eq:relax-capacity-constraints}.
%\end{proof}
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        % Need to define e, r or different presentation
        ...
        \If{$e_j \geq 0$}
        \State $\mathrm{Pred}_j \gets i$
        \State $S \gets S \cup \left\{j\right\}$
        \Else
...
% TODO: Formatting of preconditions?
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \Require $e(S) \leq r(\boldsymbol{\pi},S) \land e(t) < 0$
...
Figures \ref{algo:relaxation}, \ref{algo:relaxation-augment-flow} and \ref{algo:relaxation-update-potentials} formally describe the algorithm. We maintain a set $S$ of nodes spanned by the shortest path tree. The arcs in the tree are specified by the predecessor array, $\mathrm{Pred}$. 
...
\end{thm}
\begin{proof}
For the same reason given in \cref{thm:ssp-invariant}, the initial values of $\left(\mathbf{x},\boldsymbol{\pi}\right)$ satisfy reduced cost optimality conditions. $\mathrm{AugmentFlows}$ and $\mathrm{UpdatePotentials}$ are only invoked when their preconditions are satisfied, and so by \cref{lemma:relax-correctness-augmentflows,lemma:relax-correctness-updatepotentials} it follows that reduced cost optimality is maintained. Moreover, the main algorithm given in \cref{algo:relaxation} does not update $\mathbf{x}$ or $\boldsymbol{\pi}$ except via calls to $\mathrm{AugmentFlows}$ and $\mathrm{UpdatePotentials}$. So, reduced cost optimality is maintained as an invariant throughout the algorithm.
...
% TODO: need to mention earlier on that e and r are maintained as variables, since this affects the asymptotic complexity

\begin{lemma} \label{lemma:relax-complexity-updatepotentials}
The time complexity of $\mathrm{UpdatePotentials}$ is $O(m)$.
\end{lemma}
\begin{proof}
...
The time complexity of $\mathrm{AugmentFlow}$ is $O(m)$.
\end{lemma}
\begin{proof}
...
\begin{enumerate}[label=(\alph*)]
    \item $\mathrm{UpdatePotentials}$ is called at most $mCU$ times.
    \item $\mathrm{AugmentFlow}$ is called at most $nU$ times in between calls to $\mathrm{UpdatePotentials}$.
    \item There are at most $nmCU^2$ iterations of the outer loop on lines 2-18 of \cref{algo:relaxation}.
\end{enumerate}
\end{lemma}
\begin{proof}
By \cref{lemma:relax-dual-optimality}, $w\left(\boldsymbol{\pi}\right) \leq z^*$. A feasible flow can have a cost of at most $mCU$. By \cref{lemma:relax-correctness-updatepotentials} and \cref{assumption:integrality}, $w\left(\boldsymbol{\pi}\right)$ increases by at least one each time $\mathrm{UpdatePotentials}$ is called. Moreover, $w\left(\boldsymbol{\pi}\right)$ never decreases: $\boldsymbol{\pi}$ is only modified by $\mathrm{UpdatePotentials}$. Thus $\mathrm{UpdatePotentials}$ is called at most $mCU$ times.

As for $\mathrm{AugmentFlow}$, by \cref{lemma:relax-correctness-augmentflow} and \cref{assumption:integrality}, each call to $\mathrm{AugmentFlow}$ decreases the total excess by at least one unit. The total excess in a network can be at most $nU$. The only part of the algorithm which may \emph{increase} the excess at nodes is lines 2-3 of $\mathrm{UpdatePotentials}$. Thus, in between calls to $\mathrm{UpdatePotentials}$, at most $nU$ calls to $\mathrm{AugmentFlow}$ may be made.

For a bound on the total number of iterations, note each iteration ends after calling either $\mathrm{AugmentFlow}$ on line 12 or $\mathrm{UpdatePotentials}$ on line 16\footnotemark. Thus the number of iterations is bounded by the number of possible times either of these functions are called. By part (a), we know there are at most $mCU$ calls to $\mathrm{UpdatePotentials}$. This together with part (b) implies there are at most $nmCU^2$ calls to $\mathrm{AugmentFlow}$. This gives a bound on the number of iterations.
\footnotetext{Of course we might also call $\mathrm{UpdatePotentials}$ on line 4, but there is no guarantee this will take place and so cannot form part of the complexity analysis.}
\end{proof}
