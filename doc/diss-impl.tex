...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \While{mass balance constraints not satisfied} \label{algo:successive-shortest-path:start-loop}
          \State choose vertex $s$ with $e(s) > 0$ and vertex $t$ with $e(t) < 0$\footnotemark \label{algo:successive-shortest-path:select-active}
...
\footnotetext{Note whilst mass balance constraints are unsatisfied there must exist both a source vertex $s$ and sink vertex $t$. This is since the total sum of excesses must equal the total sum of deficits, otherwise $\sum_v b(v) \neq 0$ and the problem is infeasible.}
...
Each iteration of the algorithm identifies a source vertex $s$ and sink vertex $t$. Next, the single-source shortest-path (SSSP) problem~\cite[ch.~24]{CLRS:2009} is solved from $s$. To maintain reduced cost optimality, the potentials $\boldsymbol{\pi}$ are updated. Finally, the pseudoflow $\mathbf{x}$ is augmented along a shortest path $P$ from $s$ to $t$. The algorithm terminates when no source or sink vertices exist.
...
It is possible to modify the successive shortest path algorithm to terminate Djikstra as soon as it permanently labels a deficit vertex $l$. Although this does not affect asymptotic complexity, it may considerably improve performance in practice.\\
...
\end{lemma}
\begin{proof}
    The original proof for the lemma (given in~\cite[lemma~9.11]{Ahuja:1993}) uses the triangle inequality stated in \cref{eq:djikstra-triangle-assumption}. 
...
Any constant shift in the potential for every vertex will leave reduced costs unchanged. So we may equivalently redefine:
...
This is computationally more efficient, as we will not have to update the potential at any vertex which has not been permanently labelled (typically the majority of vertices).
...
\end{lemma}
\begin{proof}
See~\cite[lemma~9.11]{Ahuja:1993}.
\end{proof}
...
\end{cor}
\begin{proof} (Adapted from \cite[lemma~9.12]{Ahuja:1993})
...
Now, assume that reduced cost optimality holds immediately prior to execution of the loop body. The body computes the shortest path distances $s$ from a vertex $d$, updates $\boldsymbol{\pi}$ to become $\boldsymbol{\pi'}$ as defined in \cref{lemma:ssp-reduced-costs}. It then pushes flow along a shortest path from $s$ to another vertex, yielding a new flow of the same form as $\mathbf{x'}$ in \cref{cor:ssp-reduced-costs}. It follows by \cref{cor:ssp-reduced-costs} that $(\mathbf{x}',\boldsymbol{\pi}')$ satisfy reduced cost optimality at the end of the loop body. Hence, the inductive hypothesis continues to hold.
\end{proof}
...
We may use either form for $w(\boldsymbol{\pi})$ in the remainder of this section, depending on which is most convenient. Note \cref{eq:relax-obj-fun-excess} is expressed in terms of the excesses ($\mathbf{x}$-dependent) and arc costs, whereas \cref{eq:relax-obj-fun-balance} is expressed in terms of the reduced costs ($\boldsymbol{\pi}$-dependent) and balances.\\
...
So long as the pseudoflow $\mathbf{x}$ is not feasible, the algorithm selects a source vertex $s$. It then builds a tree rooted at $s$. The algorithm can perform one of two operations, described below. An operation is run as soon as its precondition is satisfied. After it is executed, the tree is destroyed and the process repeats.
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \While{network contains a source vertex $s$} \label{algo:relaxation:outer-loop-start}
        \Let{$S$}{$\Set{s}$}
...
Each iteration of the outer loop of \cref{algo:relaxation} selects a source vertex, $s$, to be the root of a tree. The inner loop adds vertices and arcs to this tree. Only non-deficit vertices and arcs with zero reduced cost are added. Paths in the tree thus correspond to shortest paths in the residual network: the distance between any vertices in the tree is zero\footnotemark.
...
If a deficit vertex $t$ is encountered, flow is augmented along the shortest path discovered from $s$ to $t$. However, an iteration may end before this. If at any point $e(S) > r(\boldsymbol{\pi},S)$, the potentials are updated, and a new iteration starts\footnotemark.
...
\Cref{algo:relaxation-augment-flow:find-P} finds a shortest path $P$ from the source vertex $s$ to a deficit vertex $t$. \Crefrange{algo:relaxation-augment-flow:compute-delta}{algo:relaxation-augment-flow:augment-path} then send as much flow as possible along path $P$, subject to:
\begin{enumerate}
    \item satisfying the capacity constraints at each arc on $P$, and
    \item ensuring $e(s) \geq 0$ and $e(t) \leq 0$.
\end{enumerate}
The restriction on $e(s)$ and $e(t)$ ensures the feasibility of the solution is improved. The excess at source vertices and deficit at sink vertices must monotonically decrease\footnotemark.
...
\Cref{algo:cost-scaling-wave-refine} describes an approach which can be proved to achieve this tighter bound for all classes of flow networks. The method maintains a list $L$ of all vertices in the network, rather than the queue $Q$ maintained by the FIFO method. The algorithm preserves the invariant that $L$ is topologically ordered with respect to the \emph{admissible graph}: the subgraph of the residual network containing only arcs with negative reduced cost.
...
\textproc{Push} operations cannot create admissible arcs, and so preserve the topological ordering. \textproc{Relabel} operations can. However, there are no admissible arcs entering $s$ immediately after $\textproc{Relabel}(s)$~\cite[lemma~6.5]{Goldberg:1987}. Thus moving $s$ to the beginning of $L$ maintains topological ordering.\\
