...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        \While{mass balance constraints not satisfied}
          \State choose node $s$ with $e(s) > 0$ and node $t$ with $e(t) < 0$
...
Initially, flow flow $\mathbf{x}$ and potentials $\boldsymbol{\pi}$ are both set to zero. Note $(\mathbf{x},\boldsymbol{\pi})=(\mathbf{0},\mathbf{0})$ satisfy reduced cost optimality. We have $G_{\boldsymbol{x}} = G_{\boldsymbol{0}} = G$, i.e. the residual and original network are the same. Moreover, all arc costs $c_{ij}$ are non-negative (by \cref{assumption:non-negative-arc-costs}) and so the reduced costs $c^{\boldsymbol{0}}_{ij}$ are also non-negative. Thus \cref{eq:optimality-reduced-cost} holds.

At each iteration, the algorithm identifies a source node $s$ and sink node $t$. It then solves the single-source shortest-path (SSSP) problem~\cite{ch.~24,CLRS:2009} from $s$, finding distances $d$ from $s$ to all other nodes and building a shortest-path tree. Let $P$ denote the path from $s$ to $t$ in this resulting tree. The pseudoflow $\mathbf{x}$ is updated by augmenting along the path $P$: push as much flow as possible, limited by (a) supply at $s$ and demand at $t$ must remain non-negative and (b) the capacities of arcs along $P$. The potentials $\boldsymbol{\pi}$ are updated, by subtracting the shortest path distances $d$. This maintains reduced cost optimality, and also ensures that the reduced arc costs are non-negative\footnotemark.
\footnotetext{This is a precondition of Djikstra's algorithm for single-source shortest path problems.}

The algorithm terminates when no source nodes or sink nodes exist. At this point, the mass balance constraints \cref{eq:mass-balance} are balanced, and so the solution $\mathbf{x}$ is feasible (see \S\ref{sec:prep-flow-pseudo}). As the algorithm moreover maintains the invariant that $(\mathbf{x},\boldsymbol{\pi})$ satisfy reduced-cost optimality, it follows that $\mathbf{x}$ is also optimal. Thus $\mathbf{x}$ is a solution to the minimum-cost flow problem.
