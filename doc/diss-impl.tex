...
So far, we have considered means by which to improve the theoretical performance of the algorithm. There has also been considerable research effort on improving the practical performance by means of \emph{heuristics} guiding the actions of the algorithm~\cite{Goldberg:1997}. This can result in improved practical performance, although it will not affect the worst-case asymptotic time complexity.

The focus of this project is in exploring new approaches to improve performance, not merely rehashing existing techniques. In light of this I decided not to implement these heuristics, judging that the development time would be better spent creating new techniques. 

A few tests require the fastest possible cost scaling implementation: for example, my comparative evaluation in \S\ref{sec:eval-comparative} benchmarks the performance of my final algorithm against state of the art implementations. For these tests, I have used Goldberg's reference implementation CS2~\cite{CS2:2009}. This solver includes all heuristics mentioned in this section, along with many other optimisations.

However for many tests, \emph{relative} performance is what is important. Cost scaling is used in the approximate solver described in \S\ref{sec:impl-approx}. The experiments in \S\ref{sec:eval-approx} seek to determine the performance speed-up from using an approximate rather than optimal solver. Of course, in a real application we would want both solvers to be as fast in absolute terms as possible. However, since the same underlying algorithm is used in both solvers, any optimisation providing an absolute performance advantage will benefit both solvers equally, leaving the ratio unchanged. During early-stage research, it is therefore better to work with simple implementations of the algorithm, which are more readily adapted to new approaches.
