...
Cost convergence is a general approach: in principle, it could be applied to any class of flow network. By contrast, this heuristic exploits the structure of flow scheduling networks.
...
The general approach to building an incremental solver is discussed in the next section, \S\ref{sec:impl-incremental-architecture}. Following this background, the suitability of each flow algorithm to the incremental problem is discussed in \S\ref{sec:impl-incremental-choice}. The \emph{augmenting path} class of algorithms, including successive shortest path and relaxation, is identified as the best choice. \S\ref{sec:impl-incremental-maintaining-rc} describes the method used to modify these algorithms. Following this, \S\ref{sec:impl-incremental-impl} gives details specific to each algorithm.
...
Based on this theoretical and practical support, I decided to implement incremental versions of successive shortest path and relaxation, described further in \S\ref{sec:impl-incremental-impl}.
...
%Work you want to discuss:
%* Google trace simulator/Firmament
%  - Online mode
%  - New termination conditions: number of events, number of rounds
%  - Hash: configurable \% of events
%  - Output graph file
%  - Collect statistics, timing
%  - Bugfixes:
%    + Don't rerun when no changes take place
%    + Avoid exporting no-op incremental deltas
%* Benchmark suite
%  - Architecture is unique, worth describing.
%  - Embellish it: significant time savings, novel approach, etc.
%  - Much of it should go in eval, though? Like:
%    + Accuracy: supports multiple iterations
%    + Test modalities
%  - Just mention the stuff that was more considerable work.

% Intro: Big it up. Something like: "Benchmarking a project of this scope is non-trivial, and requires considerable implementation. Although the details of the experiments conducted are in \S\ref{sec:eval}, this section gives some background on the implementation carried out."
% Explain general approach. Tests on pregenerated graph files, or using simulator. (Forward reference.)
% Test harness first, since it's mine.
% Then extensions to Firmament, the cluster simulator.
...
\subsection{Firmament and the cluster simulator}
