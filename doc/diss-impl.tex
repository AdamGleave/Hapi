...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \While{mass balance constraints not satisfied} \label{algo:successive-shortest-path:start-loop}
          \State choose node $s$ with $e(s) > 0$ and node $t$ with $e(t) < 0$\footnotemark \label{algo:successive-shortest-path:select-active}
          % SOMEDAY: These statements are multi-line, shorten?
          \State solve SSSP problem from $s$ to all nodes, in the residual network $G_{\mathbf{x}}$ with respect to the reduced costs $c^{\boldsymbol{\pi}}_{ij}$ \label{algo:successive-shortest-path:solve-sssp}
...
\end{thm}
\begin{proof}
Each iteration of the loop body (\crefrange{algo:successive-shortest-path:start-loop}{algo:successive-shortest-path:end-loop} of \cref{algo:successive-shortest-path}) decreases the excess of node $s$ by $\delta$ and the deficit of node $t$ by $\delta$, while leaving the excess/deficit of other nodes unchanged. Prior to the iteration, $e_s \geq 0$ and $e_t \leq 0$, so the total excess and total deficit in the network are both decreased by $\delta$. By \cref{assumption:integrality}, $\delta \geq 1$. Thus the number of iterations is bounded by the initial total excess. This in turn is bounded by $nU/2 = O(nU)$. So there are $O(nU)$ iterations in total.
...
\Cref{algo:cost-scaling-wave-refine} describes an approach which can be proved to achieve this tighter bound for all classes of flow networks. The method maintains a list $L$ of all nodes in the network, rather than the queue $Q$ maintained by the FIFO method. The algorithm preserves the invariant that $L$ is topologically ordered with respect to the \emph{admissible graph}: the subgraph of the residual network containing only arcs with negative reduced cost.
...
When describing the standard algorithms, particular emphasis was placed on their performance characteristics and theoretical properties. I made reference to this analysis in \cref{sec:impl-approx,sec:impl-incremental} when justifying the choice of algorithms used in the approximate and incremental solver.
