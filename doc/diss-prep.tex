...
Quincy and Firmament frame scheduling in terms of the minimum-cost flow problem. \emph{Jobs} are submitted to the scheduler, consisting of a number of \emph{tasks}. Flow drains from task nodes to a sink node. Along its way, it passes either through a machine node (indicating the task is to be placed there) or a special job unscheduled aggregator node (indicating the task is to be left unscheduled).

Arc capacities are used to restrict the possible scheduling states of tasks. The costs associated with arcs, by contrast, specify \emph{preferences} between possible scheduling assignments. Different scheduling policies can be encoded by varying the costs.

\subsubsection{Network structure}
%TBC: Table here would really help
%TBC: Flow graphs really useful here! Illustrate each type of network.

Each task $i$ in job $j$ is represented by a node $T_i^j$, with a single unit of supply. Machines are represented by nodes $M_l$, with arcs to a sink node $S$. Task nodes have arcs to machine nodes, with a cost specifying the preference of being scheduled on that machine.

This approach is unrealistically simple. What if there are more tasks than can be scheduled? The flow problem would become unfeasible. We introduce \emph{unscheduled aggregators} $U^j$ for each job $j$, with an arc to $S$. Each task node $T_i^j$ also has an arc to its unscheduled aggregator $U_j$, with a cost specifying the penalty for being unscheduled.

In the model above, every task has an arc to every machine. Whilst this allows for the model to be highly precise, it is very inefficient\footnotemark. Quincy makes the modification of introducing \emph{rack aggregator} nodes $R_k$, each with an arc to every machine in that rack. Moreover, there is a single \emph{cluster aggregator} $X$ with an arc to every rack aggregator\footnotemark. Each task then has a fixed number of arcs. Some of these may be to individual machines, some to rack aggregators and every task has an arc to the cluster aggregator. 
...
Every arc leaving a task node $T_i^j$ has unit capacity. Note $T_i^j$ has unit supply and no incoming arcs, so any (non-zero) capacity would be equivalent to unit capacity.

The arcs $M_l \to S$ from machine nodes to the sink node have capacity $K$, where $K$ is a parameter specifying the number of tasks which may concurrently run on a machine\footnotemark.
...
The arcs $R_k \to M_l$ from rack aggregators to nodes representing their constituent machines also have capacity $K$, since up to $K$ jobs may be scheduled on each machine in the rack via the aggregator. The arcs $X \to R_k$ have capacity $mK$, where $m$ is the number of machines in each rack.

The arcs $U_j \to S$ can be set to be uncapacitated, allowing any number of tasks in job $j$ to be unscheduled. It is possible, however, to enforce a lower bound $E_j$ and upper bound $F_j$ on the number of tasks that can be scheduled for each job. This is used in Quincy and Firmament. It can provide some degree of fairness, and in particular specifying $E_j \geq 1$ ensures starvation freedom.

Let $N_j$ be the number of tasks submitted for job $j$. To enforce the upper bound $F_j$, give $U_j$ a demand of $N_j - F_j$. At least this many tasks in job $j$ must then be left unscheduled, guaranteeing no more than $F_j$ tasks are scheduled.

To enforce the lower bound, we set the capacity on $U_j \to S$ to $F_j - E_j$. The maximum number of unscheduled tasks is thus the $N_j - F_j$ which are absorbed at $U_j$, plus the $F_j - E_j$ draining via $U_j \to S$. This gives an upper bound of $N_j - E_j$ unscheduled tasks, guaranteeing that at least $E_j$ tasks remain scheduled.

\subsubsection{Cost models}  

% Cost models: good to make clear that it's easy to encode varied scheduling policies. Can cite Malte's thesis for this. But you want to give a concrete example, or at least sketch how it can be achieved. 
% Probably not much point going into detail, here.

\subsubsection{Properties of these networks}

% e.g. m = O(n)
% Stuff you'll need for asymptotic analysis later
% Or explaining why particular algorithms work.
% Or just stuff which might be interesting; e.g. always a DAG.
